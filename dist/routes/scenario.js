...
const scenario_js_1 = require("../services/scenario.js");
...
router.post('/analyze', auth_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const analysis = await (0, scenario_js_1.analyzeScenario)(req.user.id, {
 ...
 */
router.post('/save', auth_js_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        ...
        const scenario = await (0, scenario_js_1.saveScenario)(req.user.id, { name, type, amount, category }, analysis);
        res.status(201).json({
            scenario,
            analysis,
 ...
 */
router.post('/impact', auth_js_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        const { name, type, amount, category } = req.body;
        const holisticImpact = await (0, scenario_js_1.getHolisticImpact)(req.user.id, {
            name,
            type,
            amount,
            category,
