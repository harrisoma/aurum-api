...
const vaultx_adapter_js_1 = require("../services/vaultx-adapter.js");
...
router.post('/execute', auth_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const canAfford = await vaultx_adapter_js_1.turtleBotAdapter.canAffordTrade(req.user.id, amount);
        if (!canAfford) {
            return res.status(400).json({
        ...
        // Execute the trade
        const result = await vaultx_adapter_js_1.turtleBotAdapter.executeTrade({
            userId: req.user.id,
            symbol,
            amount,
            strategy,
...
router.get('/portfolio', auth_js_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user)
            return res.status(401).json({ error: 'Unauthorized' });
        const portfolio = await vaultx_adapter_js_1.turtleBotAdapter.getPortfolio(req.user.id);
        res.json({
            portfolio,
            timestamp: new Date().toISOString(),
        });
...
router.get('/history', auth_js_1.authenticateToken, async (req, res) => {
    try {
        ...
        const history = await vaultx_adapter_js_1.turtleBotAdapter.getTradingHistory(req.user.id, parseInt(limit));
        res.json({
            trades: history,
...
//# sourceMappingURL=vaultx.js.map