...
exports.generateVaultInsights = generateVaultInsights;
exports.analyzeDecisionWithAI = analyzeDecisionWithAI;
exports.generateMonthlyRecommendations = generateMonthlyRecommendations;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const client = new sdk_1.default();
/**
 * Use Claude to generate smart financial insights
 */
async function generateVaultInsights(financialData, budgetStatus, forecastData) {
    try {
...
${budgetStatus && budgetStatus.length > 0
            ? budgetStatus.map((b) => `- ${b.category}: $${b.spentThisMonth.toFixed(2)}/$${b.budgeted.toFixed(2)} (${b.percentUsed.toFixed(0)}%)`).join('\n')
            : '- No budgets set up'}

FORECAST:
${forecastData && forecastData.length > 0
            ? forecastData.map((f) => `- ${f.category}: Projected $${f.projectedMonthEnd.toFixed(2)} (budget: $${f.budgeted.toFixed(2)})`).join('\n')
            : '- No forecast data'}
 ...
 */
async function analyzeDecisionWithAI(scenarioName, financialImpact) {
    try {
 ...
 */
async function generateMonthlyRecommendations(financialData, topCategories) {
    try {
