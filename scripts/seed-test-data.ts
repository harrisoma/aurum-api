...
async function seedTestData() {
  try {
    ...
    // Get first user from auth.users (or create one)
    ...
    for (let i = 0; i < 30; i++) {
      ...
      for (let j = 0; j < txnCount; j++) {
        const isExpense = Math.random() > 0.3; // 70% expenses, 30% transfers/income
        const category = categories[Math.floor(Math.random() * categories.length)];
        const amount = isExpense
          ? -(Math.random() * 200 + 10) // $10-$210
          : Math.random() * 500 + 50; // $50-$550
        ...
        transactions.push({
          ...
          description: `${category} expense` || 'Transfer',
    ...
    console.log(`\nYou can now test the API with user ID: ${testUserId}`);
