...
import { turtleBotAdapter } from '../services/vaultx-adapter.js';
...
router.post('/execute', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const canAfford = await turtleBotAdapter.canAffordTrade(req.user.id, amount);
    if (!canAfford) {
      return res.status(400).json({
    ...
    // Execute the trade
    const result = await turtleBotAdapter.executeTrade({
      userId: req.user.id,
      symbol,
      amount,
      strategy,
...
router.get('/portfolio', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const portfolio = await turtleBotAdapter.getPortfolio(req.user.id);
...
router.get('/history', authenticateToken, async (req: AuthRequest, res) => {
  try {
    ...
    const history = await turtleBotAdapter.getTradingHistory(
      req.user.id,
