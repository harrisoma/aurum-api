...
export const getFinancialMetrics = async (userId: string): Promise<FinancialMetrics> => {
...
export const analyzeDecisionImpact = async (
  userId: string,
  description: string,
  amount: number,
  category: string
  ...
  try {
    ...
    let affordability: 'easily' | 'challenging' | 'not possible' = 'easily';
    if (newMonthlyGap < 0) affordability = 'not possible';
    else if (newMonthlyGap < 500) affordability = 'challenging';
...
export const getAffordabilityPath = async (
  userId: string,
  amount: number,
  description: string
): Promise<string[]> => {
